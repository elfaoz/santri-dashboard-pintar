...
const ShareResultsSection: React.FC = () => {
  ...
  const getStudentData = () => {
    ...
    const memTarget = semesterMemorization.reduce((sum, r) => sum + r.target, 0);
    const memActual = semesterMemorization.reduce((sum, r) => sum + r.actual, 0);
    const memPercentage = memTarget > 0 ? Math.round((memActual / memTarget) * 100) : 0;
    ...
    // Use default daily budget of 30k
    const dailyBudget = 30000;
    const daysInSemester = currentSemester === 1 ? 184 : 181;
